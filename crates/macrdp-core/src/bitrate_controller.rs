...
impl BitrateController {
    ...
    pub fn is_lan(&self) -> bool { self.is_lan }
    ...
    pub fn update_network_type(&mut self, rtt_ewma_ms: f64) {
        if self.is_lan {
            ...
            if corrected == NetworkType::Wan {
                self.is_lan = false;
                tracing::info!(rtt_ewma_ms, "LAN → WAN: RTT exceeds threshold, activating adaptive bitrate");
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_phase1_detection() {
        assert_eq!(detect_network_phase1(IpAddr::V4(Ipv4Addr::new(192, 168, 1, 1))), NetworkType::Lan);
        assert_eq!(detect_network_phase1(IpAddr::V4(Ipv4Addr::new(8, 8, 8, 8))), NetworkType::Wan);
    }

    #[test]
    fn test_phase2_rtt_correction() {
        assert_eq!(detect_network_phase2(NetworkType::Lan, 50.0), NetworkType::Wan);
        assert_eq!(detect_network_phase2(NetworkType::Lan, 3.0), NetworkType::Lan);
        assert_eq!(detect_network_phase2(NetworkType::Wan, 3.0), NetworkType::Wan);
    }
