...
impl SCStreamDelegateTrait for StreamErrorDelegate {
    fn did_stop_with_error(&self, error: screencapturekit::error::SCError) {
        ...
        let _ = self
            .frame_tx
            .try_send(CaptureEvent::Error(error.to_string()));
    }
}
...
impl SCStreamOutputTrait for VideoOutputHandler {
    fn did_output_sample_buffer(&self, sample: CMSampleBuffer, of_type: SCStreamOutputType) {
        ...
        if let Err(tokio::sync::mpsc::error::TrySendError::Full(_)) = self.frame_tx.try_send(event)
        {
...
impl SCStreamOutputTrait for AudioOutputHandler {
    fn did_output_sample_buffer(&self, sample: CMSampleBuffer, of_type: SCStreamOutputType) {
        ...
        if let Err(tokio::sync::mpsc::error::TrySendError::Full(_)) = self.audio_tx.try_send(frame)
        {
            tracing::warn!("Audio channel full — frame dropped");
        }
    }
}
